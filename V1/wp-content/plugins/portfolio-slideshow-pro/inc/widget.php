...
class Portfolio_Slideshow_Widget extends WP_Widget {
	...
	function Portfolio_Slideshow_Widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'ps-widget', 'description' => __('Add a slideshow to any widgetized area.', 'portfolio_slideshow') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 350, 'id_base' => 'ps-widget' );

		/* Create the widget. */
		$this->WP_Widget( 'ps-widget', __('Portfolio Slideshow', 'portfolio_slideshow'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		$defaults = array( 'ID' => __('', 'portfolio_slideshow'), 'title' => __('title', 'portfolio_slideshow'), 'image_size' => 'thumbnail', 'show_image_size' => true, 'show_nav' => true );
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'ID' ); ?>"><?php _e('Slideshow ID:', 'portfolio_slideshow'); ?></label>
			<input id="<?php echo $this->get_field_id( 'ID' ); ?>" name="<?php echo $this->get_field_name( 'ID' ); ?>" value="<?php echo $instance['ID']; ?>" style="width:20%;" />
		</p>

		<!-- Image Size -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_size' ); ?>"><?php _e('Image Size:', 'portfolio_slideshow'); ?></label> 
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_nav' ); ?>">Display navigation?</label>
		</p>
